...
func readSecrets() (string, string) {
	keyID, err := ioutil.ReadFile("/var/run/secrets/deis/minio/admin/access-key-id")
	checkError(err)
	accessKey, err := ioutil.ReadFile("/var/run/secrets/deis/minio/admin/access-secret-key")
	checkError(err)
